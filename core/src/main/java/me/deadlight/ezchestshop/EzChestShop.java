...
public final class EzChestShop extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        LOGGER.info("Platform: {}/{}", ServerBuildInfo.buildInfo().brandName(), ServerBuildInfo.buildInfo().asString(StringRepresentation.VERSION_SIMPLE));

        scheduler = UniversalScheduler.getScheduler(this);
        saveDefaultConfig();

        try { Config.checkForConfigYMLupdate(); } catch (Exception e) { LOGGER.warn("Uncaught exception checking for config updates", e); }
        Config.loadConfig();
        ...
        if (economyImpl != null) { LOGGER.info("Economy: {} ({})", economyImpl.name(), economyImpl.version()); }

        LanguageManager.loadLanguages();
        try { LanguageManager.checkForLanguagesYMLupdate(); } catch (IOException e) { LOGGER.warn("Uncaught IO exception during language update", e); }

        GuiData.loadGuiData();
        try { GuiData.checkForGuiDataYMLupdate(); } catch (IOException e) { LOGGER.warn("Uncaught IO exception during GUI update", e); }
        ...
        if (Config.check_for_removed_shops) { LoadedChunksTask.startTask(); }
    ...
    private static PluginVersion findVersion(@Nullable Plugin plugin) {
        ...
        if (meta != null) return new PluginVersion(meta.getName(), meta.getVersion());
        var desc = plugin.getDescription();
        return new PluginVersion(desc.getName(), desc.getVersion());
    }
    ...
    private void registerMetrics() {
        ...
        metrics.addCustomChart(new DrilldownPie("economyProvider", () -> {
            ...
            var entry = new HashMap<String, Integer>();
            var plugin = findVersion(JavaPlugin.getProvidingPlugin(economy.getClass()));
            if (plugin != null) {
    ...
    private void registerListeners() {
        ...
        pm.registerEvents(new ChestOpeningListener(), this);
        pm.registerEvents(new BlockBreakListener(), this);
        pm.registerEvents(new BlockPlaceListener(), this);
        pm.registerEvents(new PlayerTransactionListener(), this);
        pm.registerEvents(new ChatListener(), this);
        pm.registerEvents(new BlockPistonExtendListener(), this);
        pm.registerEvents(new PlayerJoinListener(), this);
        pm.registerEvents(new ChestShopBreakPrevention(), this);

        if (Config.holodistancing) {
            pm.registerEvents(new PlayerCloseToChestListener(), this);
        } else {
            pm.registerEvents(new PlayerLookingAtChestShop(), this);
            pm.registerEvents(new PlayerLeavingListener(), this);
        }
    }

    private void registerCommands() {
        var ecs = getCommandOrThrow("ecs");
        var ecsadmin = getCommandOrThrow("ecsadmin");
    ...
    public void onDisable() {
        ...
        try {
            for (var obj : Utils.onlinePackets) {
                if (obj instanceof ASHologram h) h.destroy();
                else if (obj instanceof FloatingItem fi) fi.destroy();
            }
        } catch (Exception ignored) {}

        try {
            for (var outline : Utils.activeOutlines.values()) outline.hideOutline();
            Utils.activeOutlines.clear();
            Utils.enabledOutlines.clear();
        } catch (Exception ignored) {}
    }

    @NotNull public static EzChestShop getPlugin() { return requireNonNull(EzChestShop.INSTANCE, "Plugin instance unavailable!"); }

    private static boolean isPaperPlatform() {
        try { Class.forName("io.papermc.paper.ServerBuildInfo"); return true; }
    ...
    public DatabaseManager getDatabase() { return Utils.databaseManager; }
}
